...
package dVyper {
	...
	public class Alert {
		//
		private static var stage:DisplayObjectContainer = null;
		...
		*/
		public static function init(stageReference:DisplayObjectContainer):void {
		...
		private static function createBackground(alertOptions:AlertOptions):Sprite {
			...
			switch (alertOptions.background) {
				case "blur" :
					var BackgroundBD:BitmapData = new BitmapData(stage.width, stage.height, true, 0xFF000000+colour);
					var stageBackground:BitmapData = new BitmapData(stage.width, stage.height);
				...
				case "none" :
					...
					myBackground.graphics.drawRect(0, 0, stage.width, stage.height);
		...
		private static function getPrompt(alertOptions:AlertOptions):Sprite {
			...
			if (alertOptions.position) {
				...
				actualPrompt.x = (stage.width/2)-(actualPrompt.width/2);
				actualPrompt.y = (stage.height/2)-(actualPrompt.height/2);
			}
			return actualPrompt;
		}
		//
		...
		private static function createPrompt(alertOptions:AlertOptions):Sprite {
			...
			if (myHeight > stage.height) {
				myHeight = stage.height - 20;
				textField.autoSize = TextFieldAutoSize.NONE;
				textField.height = stage.height-40;
			}
